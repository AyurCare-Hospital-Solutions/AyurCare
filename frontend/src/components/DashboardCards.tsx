...
export const LinkCard2 = ({ name, desc, link }: { name: string, desc: string, link: string }) => {
    return <Link to={link} style={{ textDecoration: "none", overflow: "hidden" }}>
        <Paper elevation={1} sx={{ ...cardStyle, mt: 2, height: "330px", mr: "auto", minWidth: "80%", borderRadius: "15px" }}>
            <Box sx={{ my: "auto" }}>
                <Typography sx={{ fontSize: "1.75rem", fontWeight: 500 }}>{name}</Typography>
                <Typography sx={{ fontWeight: 500, }}>{desc}</Typography>
            </Box>
...
export const DateCard = () => {
    ...
    return <Paper elevation={1} sx={{ ...cardStyle, height: "150px", mb: "1px", backgroundColor: "#d9d9d9", borderRadius: "15px", mt: "10px"}}>
        <Box sx={{ my: "auto" }}>
            <Typography sx={{ fontWeight: 400 }}>Today is</Typography>
            <Typography sx={{ fontSize: "1.75rem", fontWeight: 500 }}>{dayjs(time).format("Do MMMM YYYY HH:mm:ss")}</Typography>
        </Box>
    </Paper>
}
...
export const WelcomeCard = ({ img }: { img: string }) => {
    return (
        <Paper elevation={1} sx={{ ...cardStyle, backgroundColor: "#cde1b4", mb: "1px", height: "150px", overflowX: "hidden", borderRadius: "15px", mt: "10px" }}>
            <Box sx={{ my: "auto" }}>
            ...
            <Box flexGrow="1"></Box>
            <img style={{ alignSelf: "end", width: "auto", height: "150px" }} src={img} alt="Welcome Image" />
        </Paper>
    );
}
